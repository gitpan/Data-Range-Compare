...
Data::Range::Compare - Find gaps and intersections in ranges
...
In order to use this package on your data, you will need to create 3 functions to handle computations of your data.
...
of the following numeric ranges.
  ...
  Example:
    ...
    So lets create our function to compare values.
    ...
    This function will safely compare the start and or 
...
But what happens if we want to compare ranges that are represented by letters of the alphabet?
  ...
  Example:
   ...
   So lets create a function that can compare the start and end 
   values of our ranges.
    ...
    Note: This function is slightly different than our original 
    function as we have swapped out the "<=>" operator for the 
    "cmp" operator.  This function will now safely compare the 
...
Given our numeric ranges from the "Compare start and end values" its very simple to create a function to compute the next value.

Example:

  Now we need to create a function to compute the next given
  value from a start or end range value.
   ...
   A function that can compute the next value in the case of
   numeric ranges is as simple as creating a function that just
   adds 1 to what ever number is passed into the function.
...
Example:
  ...
  This function can now compute the beginning of the next range.
  So if we wanted to compute the start of the next range for 
...
Example:
  ...
  Once again creating a function to compute the previous value
...
Once again this works fine for numeric ranges, but how would we calculate the previous range for a ranges represented by letters of the alphabet?
...
This function accepts one argument and returns the next object.  If you are implementing your own add_one, just make sure it returns the correct next value for your data.
...
This function accepts one argument and returns the next object.  If you are implementing your own sub_one, just make sure it returns the correct previous value for your data.
...
In order to compare lists of ranges for intesections, you will need to  create a list of lists containing Data::Range::Compare Objects.
...
Once the list of list refrences has been created it is possable to compare the list of ranges.
...
The above creates an anonymous subrouteen that can iterate through the list of ranges.
...
The anonymouse subrouteen does not actually skip over ranges that were not in both sets of data.  In stead creates a new missing range that represents the gap from that set of data.
