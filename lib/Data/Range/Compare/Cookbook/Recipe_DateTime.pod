...
Data::Range::Compare relies on %helper to do its internal work.  So this section explains how to populate %helper to work with DateTime.
...
"add_one" represents the next value:  With that in mind we have to do decide if our next increment is just adding 1 or moving to the next Year, Month, Day, Hour, Min, Sec etc.
...
"add_one" represents the previous value.  With that in mind we have to do decide if our next decrement is just removing 1 or moving to the previous Year, Month, Day, Hour, Min, Sec etc.
